$(function () {
	...
	$(document).on("click",".intoCar",function () {
        // this.href = "car.html";
		var proPic = $(this).parents("li").find("img").attr("src");
		//parent相邻的父级parents可越级的父级；children;相邻的子级，find可跨级的子级
		var proName =$(this).parent("dd").siblings().find("a").html();
		var proPrice = $(this).siblings().html();
		console.log(proName);
		...
		mes.price = proPrice;
		mes.pic = proPic;
		mes = "name="+mes.name+"&&"+"price="+mes.price+"&&"+"pic="+mes.pic;
		var  dataId = $(this).attr("data-id");
		window.localStorage.setItem("total",++total);
		window.localStorage.setItem(dataId,mes);

			 if(window.localStorage.getItem("id"+dataId)==null){
			var num = 0;
				 window.localStorage.setItem(("id"+dataId),++num);
			 }else{
			 var num  = window.localStorage.getItem("id"+dataId);
                 window.localStorage.setItem(("id"+dataId),++num);
			 }
